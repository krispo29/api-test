...
type Service interface {
	...
	// GetAllManifestPreImport(ctx context.Context, uploadLoggingUUID string) ([]*utils.GetDataManifestImport, error)
}
...
func (s *service) UploadPreImportManifests(ctx context.Context, uploadLogUUID string, fileBytes []byte) ([]*ResponseUploadManifestModel, error) {
	...
	freightConfig, err := s.selfRepo.GetFreightData(ctx, uploadLogUUID)
	if err != nil {
		return nil, err
	}
	...
	for j := 2; j <= len(rows); j++ {
		...
		// freightRate, _ := x.GetCellValue(sheetName, fmt.Sprintf("AQ%d", j))
		// data.FreightRate = utils.ConvertStringToFloat(freightRate)

		// freightZone, _ := x.GetCellValue(sheetName, fmt.Sprintf("AR%d", j))
		// data.FreightZone = utils.ConvertStringToFloat(freightZone)

		// amount++;
		// mawbList = append(mawbList, data.Mawb)
	...
	// // TODO:
	// err = s.selfRepo.InsertManifestData(ctx, uploadLogUUID, list, 200)
	// if err != nil {
	// 	return nil, err
	// }
...
// func (s *service) GetAllManifestPreImport(ctx context.Context, uploadLoggingUUID string) ([]*utils.GetDataManifestImport, error) {
// 	ctx, cancel := context.WithTimeout(ctx, s.contextTimeout)
// 	defer cancel()
